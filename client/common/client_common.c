...
int cfg_add_topic(struct mosq_config *cfg, client_type_t client_type, char *topic) {
  ...
  } else if (client_type == CLIENT_RESPONSE_TOPIC) {
    if (mosquitto_pub_topic_check(topic) == MOSQ_ERR_INVAL) {
...
int mosq_opts_set(struct mosquitto *mosq, struct mosq_config *cfg) {
  ...
  if (cfg->general_config->will_topic &&
    ...
    if (!cfg->general_config->quiet) fprintf(stderr, "Error: Problem setting will.\n");
  ...
  if (cfg->general_config->username &&
    ...
    if (!cfg->general_config->quiet) fprintf(stderr, "Error: Problem setting username and password.\n");
    mosquitto_lib_cleanup();
    return EXIT_FAILURE;
  }
#ifdef WITH_TLS
  if (cfg->tls_config->cafile || cfg->tls_config->capath) {
    ...
    if (ret) {
      if (ret == MOSQ_ERR_INVAL) {
        if (!cfg->general_config->quiet) fprintf(stderr, "Error: Problem setting TLS options: File not found.\n");
      } else {
  ...
  if (cfg->tls_config->insecure && mosquitto_tls_insecure_set(mosq, true)) {
    if (!cfg->general_config->quiet) fprintf(stderr, "Error: Problem setting TLS insecure option.\n");
    mosquitto_lib_cleanup();
    return EXIT_FAILURE;
  }
  ...
  if (cfg->tls_config->tls_alpn && mosquitto_string_option(mosq, MOSQ_OPT_TLS_ALPN, cfg->tls_config->tls_alpn)) {
    if (!cfg->general_config->quiet) fprintf(stderr, "Error: Problem setting TLS ALPN protocol.\n");
    mosquitto_lib_cleanup();
    return EXIT_FAILURE;
  }
#ifdef FINAL_WITH_TLS_PSK
  if (cfg->tls_config->psk && mosquitto_tls_psk_set(mosq, cfg->tls_config->psk, cfg->tls_config->psk_identity, NULL)) {
    if (!cfg->general_config->quiet) fprintf(stderr, "Error: Problem setting TLS-PSK options.\n");
    mosquitto_lib_cleanup();
    return EXIT_FAILURE;
  }
#endif
